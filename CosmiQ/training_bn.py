...
from unet_batchnorm import WrappedModel, UNet2Sigmoid
...
class train:
    ...
    def __init__(self, image, mask=None, ignore=None, sky=None, mode='pair', aug_sky=(0, 0), aug_img=(1, 1), noise=False, saturation=1e5,
        ...
        if gpu:
            ...
            self.network = nn.DataParallel(UNet(normalization_layer='batch_norm', hidden=hidden))
            ...
            self.network = WrappedModel(UNet(normalization_layer='batch_norm', hidden=hidden))
            #self.network.to(self.device)
            self.network.type(self.dtype)
    ...
    def set_input(self, img0, mask, ignore):
        ...
        #self.img0 = img0.to(device=self.device, dtype=torch.float32).view(-1, 1, self.shape, self.shape)
        self.img0 = Variable(img0.type(self.dtype)).view(-1,1, self.shape, self.shape)
        #self.mask = mask.to(device=self.device, dtype=torch.float32).view(-1, 1, self.shape, self.shape)
        self.mask = Variable(mask.type(self.dtype)).view(-1,1, self.shape, self.shape)
        #self.ignore = ignore.to(device=self.device, dtype=torch.float32).view(-1, 1, self.shape, self.shape)
